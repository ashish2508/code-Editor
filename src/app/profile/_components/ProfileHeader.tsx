...
function ProfileHeader({ userStats, userData, user }: ProfileHeaderProps) {
	...
	return (
		<div
			className="relative mb-8 bg-gradient-to-b from-stone-600/30 to-zinc-900/30 rounded-2xl p-8 border
			...
			<div className="relative flex items-center gap-8">
				...
				<div>
					<div className="flex items-center gap-3 mb-2">
						...
						{userData.isPro && (
							<span className="px-3 py-1 bg-teal-500/10 text-lime-400 rounded-full text-sm font-medium">
								Pro Member
							</span>
						)}
					</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
				{STATS.map((stat, index) => (
					<motion.div
						...
						key={index}
						className="group relative bg-gradient-to-br from-zinc-950/70 to-slate-950/20 rounded-2xl overflow-hidden"
					>
						{/* Glow effect */}
						<div
